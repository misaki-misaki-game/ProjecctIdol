...
public class ButtonScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && gameStart) // 左クリックしたら
        {
            ...
            if (hitSprite == true) // レイが当たったら
            {
                ...
                cheakSignal(clickedGameObject); // チェイン確認関数を呼び出し
                clickedGameObject.GetComponent<SignalScript>().BreakSignal(); // ブレイク関数を呼び出し
    ...
    public void cheakSignal(GameObject gameObject) // クリックされたオブジェクトの8方向をチェックし、チェインを確認する関数
    {
        ...
        {
            // クリックしたオブジェクトの1つ右のSignalScriptを格納
        ...
        {
            // クリックしたオブジェクトの1つ左のSignalScriptを格納
            comparisonSignalSS = signals[centerSignalTP * 10 + centerSignalDP - 1].GetComponent<SignalScript>();
            if (centerSignalSS.state == comparisonSignalSS.state) // stateが同じなら
            {
                chain++; // チェインカウントを1増やす
        ...
        if (centerSignalTP > 0 && centerSignalDP < 4) // クリックしたオブジェクトの列が最上段かつ行が右端でなければ
        {
        ...
        if (centerSignalTP > 0 && centerSignalDP > 0) // クリックしたオブジェクトの列が最上段かつ行が左端でなければ
        {
        ...
        if (centerSignalTP < 5 && centerSignalDP < 4) // クリックしたオブジェクトの列が最下段かつ行が右端でなければ
        {
        ...
        if (centerSignalTP < 5 && centerSignalDP > 0) // クリックしたオブジェクトの列が最下段かつ行が左端でなければ
        {
